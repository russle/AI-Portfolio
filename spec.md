...
* **頁面功能**：提供強大的資產再平衡決策支援。**本頁面採取純粹的決策模擬沙盒設計，不提供任何會自動覆寫大類資產帳目的寫入按鈕。使用者需參考此處的交易方向與金額建議，前往實體券商交易，並在首頁嚴格手動登載最新快照明細。這能極大化保護帳面真實數據結構的完整性，防範數據失衡。**
...
* **頁面功能**：情境分析與壓力測試沙盒。100% 安全獨立的「模擬沙盒預演環境」，在不污染真實 context 狀態的情況下，一鍵預覽「市場暴跌 -10%」、「美股大牛市 +20%」、「美元飆升至 35 TWD」及「惡性通膨至 5%」對退休成功率與未來資產的衝衝擊。**本頁面採取完全唯讀/沙盒設計，杜絕任何將模擬情境覆寫、污染真實帳戶資產數據的風險，確保使用者歷史記錄數據與真實追蹤軌跡的絕對安全性。**
...
#### 5. 提領風險與代價雙向防呆提示 (Withdrawal Risk & UX Anti-Fool Proofing) [NEW]
為了防範非財務專業的使用者被 Die to Zero (花光為止) 法則的「100% 成功率」與「85歲前安全無虞」的字面數據所誤導，忽略其代價是生活費會隨股市下跌而被動調降；以及 4% 法則在熊市底部剛性割肉本金枯竭的實質風險，系統在 P5 保守極端卡片中引進了「智慧條件式雙向防呆對比」：
* **Die to Zero (花光為止) 的防呆機制**：
  - **智慧標題大字**：當 `withdrawalRule === 'die_to_zero'` 時，若 P5 軌跡因年金均攤未提前歸零（`depletionAge_P5` 為 `null`），標題大字由原本容易產生假性安全感的 `🛡️ 85歲前安全無虞` 被動替換為 **`🛡️ 被動動態重均攤`**。
  - **智慧描述文案**：切換為精確財務警告：`💡 極端低迷行情下，資產絕不提前枯竭。但代價是您的生活費會被大盤暴跌【動態腰斬/收縮】，晚年實質生活水平將顯著調降。`。
  - **指南橫幅 Alert 警告**：在財富自由提領指南卡片中，額外注入高對比度黃色警示橫幅（配備 `AlertTriangle` 圖標與脈衝動畫），提示使用者長壽與熊市腰斬的複合威脅。
  - **底部警告徽章**：`⚠️ 警惕：成功率 100% 的背後是犧牲生活品質，熊市下每月開銷將極度收縮。`。
* **4% 提領法則 (定額剛性) 的防護機制**：
  - **底部割肉警告**：當選擇 `four_percent` 且在極端市場（P5 軌跡）下出現資產耗盡時，底部自動渲染高亮度紅色警示框：`⚠️ 剛性提領在熊市底部割肉賣股加速枯竭，建議考慮 GK 動態護欄或 CAPE 估值避險。`。
...
## 6. 四大黃金功能優化技術規格 (2026年5月重大升級) [NEW]

本系統於 2026 年 5 月完成了四項神級功能優化整合，顯著提升了數據分析的顆粒度、交易模擬的實用性以及退休防禦的安全評估維度。

### 6.1 ⚖️ 智慧下單機三軌入金分配器
* **路由位置**：`/#/order` (`OrderPage.tsx`)
* **核心功能**：支持新增新資金（儲蓄入金）下的股數換算，提供三種獨立交易軌道：
  1. **智慧入金再平衡**：動態調用 `calculateCashOnlyRebalance`。在不賣出任何現有資產的前提下，「只買不賣」地將新資金精準分配給當前低配最嚴重的部位，一鍵算出應買股數。
  2. **新資金定比分配**：直接按目標配置比例 $Target_i$ 將新資金分配至各大類，不考慮當前的實際漂移狀態。
  3. **雙向精準再平衡**：維持原有的賣高買低精準調整建議。
* **沙盒安全機制**：本工具採完全唯讀沙盒設計，計算結果絕不覆寫或污染真實持股現值，保障追蹤數據的絕對安全性。

### 6.2 📈 總覽頁「本金線 vs 市值線」對抗折線與五卡 ROI 指標牆
* **路由位置**：`/#/` (`OverviewPage.tsx`)
* **數據基礎**：在 `PortfolioHistoryPoint` 中擴增 `cumulative_investment` (累計投入本金) 欄位。補記歷史快照 Modal 支援使用者選填各月的累計投入本金。
* **雙線漸變發光圖表**：Recharts 折線圖中同時渲染「資產總市值線」（藍色發光實線）與「累計投入本金線」（橙色發光虛線），直觀對比出隨時間推移產生的「複利剪刀差（累計損益）」。
* **五卡財務戰情牆**：首頁頂部升級為 `lg:grid-cols-5` 的高質感五卡片佈局：
  - **最新淨資產** (藍色)：當前市值加總。
  - **累計投入本金** (灰色)：歷史儲蓄投入本金。
  - **累計損益與總報酬** (綠紅雙向)：市值與本金的剪刀差及總 ROI %。
  - **CAGR 歷史年化報酬** (紫色)：基於歷史首尾點幾何精算。
  - **FIRE 目標進度** (青色)：當前淨值相較退休支出 25 倍目標的進度條。

### 6.3 📊 總覽頁細分資產版圖消長堆疊面積圖
* **路由位置**：`/#/` (`OverviewPage.tsx`)
* **高級視覺切換**：歷史圖表上方提供 `📈 本金與市值對照` 與 `📊 資產消長堆疊` 切換按鈕。
* **堆疊面積圖**：切換為 `<AreaChart>`，採用各大類專屬 HSL 配色方案（台股-藍、美股-紫、基金-綠、加密幣-黃、現金-灰）進行月度配置消長堆疊渲染，極具震撼的視覺效果。

### 6.4 🦖 退休規劃頁歷史黑天鵝生存壓力測試沙盒
* **路由位置**：`/#/retirement` (`RetirementPage.tsx` / `src/utils/retirement.ts`)
* **危機場景載入**：內建歷史上最著名的三大黑天鵝波段大類資產真實年度收益率序列（2000 科技股破滅、2008 全球金融海嘯、2022 股債雙殺高通膨）。
* **生存回測引擎 (`runRetirementCrisisBacktest`)**：
  - **配置自適應**：有真實資產時，智慧讀取當前「真實持股大類佔比」；無真實持股時，自動採用「配置目標比例」進行回測。
  - **初始金自適應**：有真實資產時，以當前淨資產為退休初始本金；無真實資產時，以 1000 萬 TWD 初始本金為預設進行壓力測試。
  - **逐月演算**：跑 10 年 (120 個月) 的提領消耗期。支援通膨逐月調整、支出微笑曲線折減，以及 GK 動態提領護欄。
  - **生存診斷**：以 Recharts 紅色發光折線繪製 10 年資產消耗軌跡，並智慧診斷輸出「您的退休配置已成功熬過考驗」或「退休金在第 X 個月宣告歸零」，給予最硬核的避險防禦指導。